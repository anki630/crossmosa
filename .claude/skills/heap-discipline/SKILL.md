...
The constraint that makes every call matter: ~380KB RAM, no PSRAM, one ~52KB
framebuffer (X3: 792×528÷8 = 52,272 bytes — the 800×480/48KB figure floating
around upstream is the X4 panel, not this project's target). **Fragmentation,
not total usage, is what kills this device.**
...
- Large contiguous blocks fragment worst. Full-screen-class buffers use the
  ...
  never demand one contiguous ~52KB block. Reuse that path. Do not malloc a
