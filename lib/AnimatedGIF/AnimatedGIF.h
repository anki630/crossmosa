...
#else
// CrossMosa v162：480 → 3072。電子書整頁掃描圖輕易超過 480（實測 Sampler 的地圖 528 寬），
// 3072 對齊 ImageToFramebufferDecoder::MAX_SOURCE_PIXELS（3072×2048）的幾何上限。
// 代價只是 GIFIMAGE 內的單行緩衝 +2.6KB（結構總計約 28KB，單次解碼期間的暫態配置）。
...
typedef struct gif_image_tag
{
    ...
    int iLocalPalSize, iGlobalPalSize;
