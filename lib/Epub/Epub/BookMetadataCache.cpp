...
static std::string clampSerializedString(const std::string& s) {
  ...
  return s.substr(0, safeLen > 0 ? static_cast<size_t>(safeLen) : 0);
}
...
void BookMetadataCache::createSpineEntry(const std::string& href) {
  ...
  const SpineEntry entry(clampSerializedString(href), 0, -1);
  if (passOut) {
...
void BookMetadataCache::createTocEntry(const std::string& title, const std::string& href, const std::string& anchor,
  ...
  const TocEntry entry(clampSerializedString(utf8ComposeNfc(title)), clampSerializedString(href),
                       clampSerializedString(anchor), level, spineIndex);
  if (passOut) {
