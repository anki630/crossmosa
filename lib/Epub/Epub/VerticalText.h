...
// ⚠️ **v284 更正**：原本這裡寫著「曾規劃重用既有的 `lineSpacing`，已放棄 ——
//    `getReaderLineCompression()` 的值域會隨字型家族重疊，反推必錯」。**那個理由消失了**：
//    v284 把橫排行距也改成 em 的倍數，而且**直接沿用下面這三個常數**。
//    兩個設定項仍然分開（使用者可能想讓橫排與直排鬆緊不同），但它們現在是同一把尺。
// ⭐⭐ **下面三個數字現在同時定義直排的欄距與橫排的行距 —— 改它要想到兩邊。**
//
