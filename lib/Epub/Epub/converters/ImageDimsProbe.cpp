...
bool ImageDimsProbe::feed(const uint8_t b) {
  switch (state) {
    ...
    case State::GifHeader:
      // Bytes 1..3: "IF8"; byte 4: '7'|'9'; byte 5: 'a'; 6..7: width LE; 8..9: height LE.
      if (pos < 4) {
        if (b != "GIF8"[pos]) {
          state = State::Failed;
          return false;
        ...
        width |= (static_cast<uint32_t>(b) << 8);
      } else if (pos == 8) {
        height = b;
      } else if (pos == 9) {
        height |= (static_cast<uint32_t>(b) << 8);
        state = State::Done;
        pos++;
        return false;
      }
