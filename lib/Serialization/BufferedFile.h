...
inline void readString(BufferedFileReader& in, std::string& s) {
  uint32_t len;
  readPod(in, len);
  // v53 壞檔守衛(同 Serialization.h::MAX_SERIALIZED_STRING):SD 上一個位元翻轉會讓
  // resize 要求上百 MB → -fno-exceptions 下 abort()。截斷成空字串,讓裝置活著。
  if (len > serialization::MAX_SERIALIZED_STRING) {
    // 跳過 payload 維持串流同步(見 Serialization.h::readString)。
    // 32-bit 迴繞防護:len 接近 UINT32_MAX 時 position()+len 會繞回更小的值,
    // 變成「合法的向後 seek」——寧可不跳,也不能倒退重讀。
    s.clear();
    const size_t target = in.position() + len;
    if (target >= in.position()) in.seek(target);
    return;
  }
  s.resize(len);
  if (len > 0) {
