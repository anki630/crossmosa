...
/* CrossMosa divergence from upstream (patch 4 of 4) -- 256 -> 32.
 *
 * WHY: this array is embedded twice in every `struct smb2_pdu` (its `in` and
 * `out` smb2_io_vectors) and twice more in every `struct smb2_context`, so its
 * size is multiplied by every request in flight and every connection. At 256 a
 * single in-flight PDU costs ~12.5 KB on a 64-bit host and ~6.3 KB on this
 * device's 32-bit RISC-V core -- against a largest contiguous free block of
 * 40,948 bytes at the moment the SMB server starts. That is room for about six
 * PDUs, while libsmb2's own server grants the client 128 credits.
 *
 * The device said so out loud, in diag20.log, mid-copy from an iPhone:
 *     SMB alloc FAILED site=error_reply cmd=6 status=0xc0000128,
 *                                        largest free block 4596
 *     SMB service(in) FAILED ...: Read from socket failed, errno:9
 * -- the server could not allocate a PDU for a six-byte error reply, so the
 * request got no answer at all and the connection died. The same shortage
 * produced `accept FAILED: err=-12` on a third connection in diag19.log. One
 * browse took the largest free block from 40,948 down to 4,596.
 *
 * WHY 32, AND NOT A GUESS: instrumented `smb2_add_iovector()` in a throwaway
 * copy of this tree and recorded the high-water mark of `v->niov` across the
 * whole workload -- the 73-check desktop suite, a 400 KB file copied in and
 * read back, and the compound CREATE+QUERY_INFO+CLOSE an iPhone actually
 * sends. **The high-water mark was 8.** 32 is four times that.
 *
 * MEASURED EFFECT (x86-64; the device's 32-bit build halves both columns, the
 * ratio holds): smb2_io_vectors 6,168 -> 792, smb2_pdu 12,568 -> 1,816,
 * smb2_context 7,256 -> 1,880. Re-ran the 73-check suite and the 400 KB
 * round trip at 32: all pass, SHA-256 exact, high-water still 8.
 *
 * FAILURE MODE IF EVER EXCEEDED: smb2_add_iovector() bounds-checks
 * (init.c, `if (v->niov >= SMB2_MAX_VECTORS)`), sets "Too many I/O vectors"
 * and returns NULL -- a clean refusal, not a corruption. It appears in
 * diag.log as a failed command, not as damage.
 *
 * ⚠️ That was FALSE until v80: it also freed the caller's buffer, which nine
 * call sites free again. Measured: 10 compounded WRITEs worked, 11 aborted
 * with "free(): double free detected", client's choice of count. Divergence #5
 * removes the free. The sentence above is true NOW because we made it true.
 *
 * ON UPGRADING libsmb2: re-measure the high-water mark before assuming 32 is
 * still enough -- a new encoder that emits more vectors per PDU is exactly the
 * kind of change this pin cannot see. The instrumentation is four lines in
 * smb2_add_iovector(); the procedure is in docs/third-party/libsmb2-vendoring.md.
 */
