                                    ...
                                    void (*free_cb)(void *))
{
                ...
                if (v->niov >= SMB2_MAX_VECTORS) {
                        ...
                        /*
                         * ========= CrossMosa divergence #5 (approved) =========
                         * Upstream freed the caller's buffer here:
                         *
                         *     if (free_cb && buf) { free_cb(buf); }
                         *
                         * That is a DOUBLE FREE, not a leak fix. Nine call sites
                         * free the same pointer again when this returns NULL:
                         * smb2-cmd-query-directory.c:119, smb2-cmd-set-info.c:92,
                         * and socket.c:432, 522, 576, 624, 669, 709, 758.
                         * Measured on the desktop harness: compounding 10 WRITEs
                         * into one packet works, 11 aborts with "free(): double
                         * free detected". The compound count is chosen by the
                         * CLIENT, and on this device's TLSF allocator the same
                         * sequence is heap corruption rather than a clean abort.
                         * (docs/third-party/libsmb2-vendoring.md and
                         * libsmb2-private.h both claimed this overflow was "a
                         * clean refusal, not a corruption". It is not.)
                         *
                         * Fixed HERE rather than at the nine call sites because
                         * the other 66 sites pass a free_cb and do NOT free on
                         * NULL -- they rely on this branch. So the choice is
                         * 9 double frees or 66 leaks-on-overflow, and the leak
                         * is the right direction: it is bounded (the request
                         * fails and the file-transfer activity ends in
                         * silentRestart()), it never touches an allocation that
                         * belongs to something else, and it restores the
                         * ordinary C contract -- on failure ownership stays with
                         * the caller.
                         *
                         * For anyone upstreaming: the real fix is to make all
                         * 75 sites agree on one contract. This is the safe half.
                         * ======================================================
                         */
                        (void)free_cb;
                        (void)buf;
