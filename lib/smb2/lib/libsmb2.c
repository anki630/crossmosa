...
/* ============================================================================
 * CrossMosa addition -- appended below the upstream file unmodified above
 * this banner. Nothing above this point has been changed; do not add code
 * anywhere except after this comment. See docs/third-party/libsmb2-vendoring.md
 * ("Vendoring status: upstream + one appended function") for the full
 * rationale and scripts/verify_libsmb2_patch.py, which checks that this file
 * differs from pristine upstream in exactly this one hunk.
 *
 * Why this exists: CrossMosa's SmbServer (src/network/SmbServer.cpp) drives
 * libsmb2 without a dedicated thread -- a single activity loop on an ESP32-C3
 * with no FreeRTOS task calls SmbServer::tick() once per iteration, and
 * tick() must never block (that loop also has to keep driving the e-ink
 * display; this device has no serial port, so a hung task is nearly
 * undebuggable). Upstream's only server-side reference loop,
 * smb2_serve_port() (above in this same file), owns a blocking do-while with
 * its own select() and is designed to run in a dedicated thread -- it cannot
 * be adapted into a "call once, return immediately" shape from outside this
 * file.
 *
 * Specifically, the one piece SmbServer.cpp cannot do itself is finishing
 * the bootstrap of a freshly-accepted connection (the block at
 * smb2_serve_port() above, "got a new smb2 context with a connection..."):
 * it needs `struct connect_data`, a type defined nowhere outside this file
 * (not even opaquely in lib/smb2/include/libsmb2-private.h -- see that
 * header's own forward declaration, "struct connect_data; (comment: defined
 * in libsmb2.c)"), and smb2_negotiate_request_cb, declared `static` above in
 * this same file (zero external linkage -- not a documentation gap, a
 * C-language fact). SmbServer.cpp already does the accept itself via the
 * existing public smb2_serve_port_async(); this function does only the
 * remaining bootstrap step, using the exact same calls upstream's own
 * smb2_serve_port() makes, just factored out into a callable, exported
 * function instead of being inlined in that one function's loop body.
 *
 * Deliberately NOT reproduced here: upstream's inline version does not
 * `return`/bail out after `c_data == NULL` before doing
 * `c_data->server_context = server` -- i.e. it would dereference a null
 * pointer on a calloc failure. That isn't something this new function
 * should carry forward: this is new code, not a byte-for-byte copy of
 * existing (already-shipped, less scrutinized) upstream code, so it returns
 * immediately on that failure instead.
 *
 * Deliberately NOT included here: the `cb`/`cb_data` "new connection"
 * notification upstream's smb2_serve_port() invokes after a successful
 * bootstrap. SmbServer doesn't use that generic callback mechanism (it has
 * no such parameter), so this function stays scoped to exactly what
 * SmbServer::tick() needs, per this project's own scope-discipline norms --
 * not a general-purpose replacement API for smb2_serve_port_async()'s
 * remaining gap.
 * ========================================================================== */

int
crossmosa_smb2_finish_accept(struct smb2_context *smb2, struct smb2_server *server)
{
        struct connect_data *c_data;

        if (smb2 == NULL || server == NULL) {
                return -EINVAL;
        }

        c_data = calloc(1, sizeof(struct connect_data));
        if (c_data == NULL) {
                smb2_set_error(smb2, "crossmosa_smb2_finish_accept: Failed to allocate connect_data");
                smb2_close_context(smb2);
                return -ENOMEM;
        }
        c_data->server_context = server;
        ...
        /* alloc a pdu for first server request -- identical call to the one
         * smb2_serve_port() makes inline above */
        smb2->pdu = smb2_allocate_pdu(smb2, SMB2_NEGOTIATE, smb2_negotiate_request_cb, c_data);
        if (smb2->pdu == NULL) {
                smb2_set_error(smb2, "crossmosa_smb2_finish_accept: can not alloc pdu for request");
                smb2_close_context(smb2);
                return -ENOMEM;
        }

        /* got a new smb2 context with a connection -- enlist it */
        smb2->owning_server = server;
        smb2->max_transact_size = server->max_transact_size;
        smb2->max_read_size     = server->max_read_size;
        smb2->max_write_size    = server->max_write_size;
