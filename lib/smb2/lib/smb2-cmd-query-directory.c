        ...
        if (smb2->passthrough) {
                ...
                do {
                        ...
                        if (in_remain >= SMB2_FILEID_BOTH_DIRECTORY_INFORMATION_SIZE) {
                                /* ================= CrossMosa in-line fix to an upstream bug =================
                                 * Upstream wrote `offset + fs_size` here. `offset` is the running byte
                                 * position of the CURRENT entry from the start of the reply buffer, so that
                                 * emitted an ABSOLUTE offset -- but MS-FSCC 2.4.8 defines NextEntryOffset as
                                 * "the byte offset from the beginning of THIS entry to the beginning of the
                                 * next entry", i.e. a RELATIVE distance. Entry 1 was correct only by accident
                                 * (its offset is 0); from entry 2 on, a client walking `p += NextEntryOffset`
                                 * overshoots by the accumulated prefix and silently skips files.
                                 *
                                 * This is a defect, not a house convention: libsmb2's OWN client walks
                                 * relatively (libsmb2.c, `offset += fs.next_entry_offset` in the readdir
                                 * decode loop), so upstream's server cannot feed upstream's client. So do
                                 * smbprotocol and every other conformant client.
                                 *
                                 * See docs/third-party/libsmb2-vendoring.md ("The second patch") for the
                                 * measurement that found it, and scripts/verify_libsmb2_patch.py, which pins
                                 * this exact region -- any other edit to this file fails the check.
                                 * ========================================================================== */
