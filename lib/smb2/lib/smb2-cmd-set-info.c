                            ...
                            struct smb2_pdu *pdu)
{
         ...
         * See docs/third-party/libsmb2-vendoring.md, "The third patch".
         *
         * Upstream guards the two lines below with
         *
         *     if (!smb2->passthrough) {
         *             smb2_set_error(smb2,
         *                     "can not interpret set-info buffers yet");
         *             return -1;
         *     }
         *
         * so a server that is not in passthrough mode refuses every SET_INFO
         * that carries a buffer -- which is every real one. Upstream's own
         * wording is "can not interpret set-info buffers YET": SET_INFO is
         * simply unimplemented on the server path, not invalid.
         *
         * THE REASON THIS COULD NOT BE WORKED AROUND IN A HANDLER: that -1
         * is returned from a DECODE function, not from a handler.
         * smb2_process_payload_variable() (pdu.c:1101) propagates it out of
         * smb2_read_from_socket(), which TEARS THE CONNECTION DOWN -- where
         * the same -1 returned from handlers->set_info_cmd would have
         * produced a clean per-request STATUS_NOT_IMPLEMENTED and left the
         * session alive (libsmb2.c:3883-3893). There is no callback between
         * the two points, so no handler can intervene. Measured on this
         * project's desktop harness: one rename request kills the whole
         * session, and destruction_event fires mid-copy.
         *
         * And this is the PRIMARY use case, not an optional extra: macOS and
         * iOS send SET_INFO after writing a file -- FILE_BASIC_INFORMATION to
         * stamp timestamps, FILE_DISPOSITION_INFORMATION to delete,
         * FILE_END_OF_FILE_INFORMATION to set size -- so an iPhone would be
         * disconnected at the end of its first copy.
         *
         * The fix is to do unconditionally what the passthrough branch
         * already did: hand the raw buffer to the server's handler and let it
         * interpret the info class (see set_info_cmd in
         * src/network/SmbFileHandlers.cpp). Server-only by construction: a
         * client never RECEIVES a SET_INFO request, so this function is
         * unreachable in client mode and no client path can be affected.
         * =================================================================
         */
