...
def generate_keys_header(
    ...
    # CrossMosa L5：只保留實際存在的語言。上面那段註解說得對——列表裡的 Language
    # 值被移除時「will fail to compile」是刻意的絆線。但我們是【整批】移除語言，
    # 絆線每次都會響，而 V1_LANGUAGES 全樹沒有任何使用者（language.bin 遷移未接線）。
    # 所以改成取交集：絆線對【剩下的】語言仍然有效，不會因為批次移除而失去意義。
    _v1_frozen = [
        "EN", "ES", "FR", "DE", "CS", "PT", "RU", "SV", "RO", "CA", "UK",
        "BE", "IT", "PL", "FI", "DA", "NL", "TR", "KK", "HU", "LT", "SI",
    ]
    v1_codes = [c for c in _v1_frozen if c in languages]
