...
void CrossPointSettings::toJson(JsonDocument& doc) const {
  ...
  // 才【條件式插入】這個條目 —— 泛用迴圈走的是 getSettingsList()，所以 IMU 探測失敗那次
  // 存檔就會把使用者的值永久抹掉（saveToFile 開新 JsonDocument 重寫整個檔，不是 merge）。
...
bool CrossPointSettings::fromJson(JsonVariantConst doc) {
  ...
  // v182/v184：介面主題枚舉兩度重排（RoundedRaff、Classic 退役）。帶 uiThemeSchema=2 的存檔已是新編號；
  // 沒有 schema 的舊存檔依舊編號遷移：0 Classic→Formosa、1 Formosa→0、2 Extended→1、3（RoundedRaff／v182 Pro）→Pro、
  // 4（v179–v181 Pro）→Pro。泛用迴圈已先做 clamp，這裡照原始值補回。
  {
    const uint8_t schema = doc["uiThemeSchema"] | static_cast<uint8_t>(0);
    if (schema < 2) {
      const uint8_t rawTheme = doc["uiTheme"] | static_cast<uint8_t>(1);
      switch (rawTheme) {
        case 0:
        case 1:
          s.uiTheme = LYRA;
          break;
        case 2:
          s.uiTheme = LYRA_3_COVERS;
          break;
        default:
          s.uiTheme = FORMOSA_PRO;
          break;
      }
    }
  }
  ...
  // Front button remap — managed by RemapFrontButtons sub-activity, not in SettingsList.
  // v57：tiltPageTurn 手動載入（存檔端同理，見 toJson）。
  tiltPageTurn = clamp(doc["tiltPageTurn"] | (uint8_t)TILT_OFF, TILT_PAGE_TURN_COUNT, TILT_OFF);
...
#ifdef OMIT_FONTS
  // CrossMosa L2：OMIT_FONTS 之下只有 notoserif14（資料指向 notosans_14）被 insert。
  // 其餘七個 builtin ID 都沒進 fontMap —— 直接回傳它們會 miss，GfxRenderer 只記一行 log
  // 然後畫出空白（內文整頁不見）。所有 family/size 組合一律映射到唯一存在的那一個。
  // ⚠️ 上游沒有守這裡；把 OMIT_FONTS 加進 build_flags 而不補這段，內建字型路徑就是壞的。
