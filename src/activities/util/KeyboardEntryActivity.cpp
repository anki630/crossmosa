...
fui::KeyboardLayoutId layoutForLanguage(const Language /*language*/) {
  // CrossMosa L5：只保留 english 與 zh-hant 兩個語系，Language::FR/DE/ES 這幾個
  // enum 值已不存在（I18nKeys.h 由 gen_i18n.py 依 translations/ 底下的 yaml 生成）。
  // 中文沒有對應的實體鍵盤配置，一律用 QWERTY。
  // 要還原分語言配置：把對應的 yaml 加回 translations/，再把 switch 寫回來。
...
void KeyboardEntryActivity::render(RenderLock&&) {
    ...
    if (cursorMode) {
      ...
      if (inputType == InputType::Password && togglePos) {
        ...
        if (inputType == InputType::Password) {
          ...
          renderer.drawCenteredText(UI_10_FONT_ID, hintLineY, passTip, true);
        }
      }
    } else {
      renderer.drawCenteredText(UI_10_FONT_ID, hintY, tr(STR_KB_HINT_EDIT_ENTRY), true);
    }
  }
  ...
  // Formosa：鍵圓角走主題 metric（fui 預設 0 = 直角），不動 SDK
  props.keyRadius = static_cast<uint8_t>(metrics.keyboardKeyCornerRadius);
