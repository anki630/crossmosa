...
// v36: hangs off the boot-resolved data dir; built on first use (dictionary
// lookups happen long after DataDir::resolve()).
const char* dictTmpFile() {
  static char p[32] = "";
  if (!p[0]) snprintf(p, sizeof(p), "%s/dict.tmp", DataDir::path());
  return p;
}
...
bool Dictionary::readDefinition(const DictLocation& location, std::string& out) {
  ...
  if (hasPlainDict) {
    ...
    HalFile tmp = Storage.open(dictTmpFile(), O_WRITE | O_CREAT | O_TRUNC);
    if (!tmp) {
      LOG_ERR("DICT", "Failed to open %s", dictTmpFile());
      return false;
    }
